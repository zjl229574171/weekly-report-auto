...
版本：权威版本号以 Sheet《微波周报数据》「流程版本」子表为准；git tag 作记录（当前 v0.5.10）。版本号三段式「发布.大修.小更」：发布段=对外发布计数（当前 0，未发布）；大修段=影响使用的改动；小更段=不影响使用的改动（进位规则见「二A」）。腾讯文档备份链接见本地 manifest.json（仅备份/参考，权威以本仓库为准）
...
| 版本不同（三段中任一段不同均适用） | 单轮询问：向用户一句话说明"检测到新版本（本地 0.x.y → 云端 0.a.b，更新说明：…）。是否更新？"用户答复更新 → git pull（拉取流程文档最新版）→ 更新本地 manifest.json 的 version 字段 → 一句话告知完成；用户答复不更新 → 继续用本地版本跑业务。只问这一轮 |
