...
版本：权威版本号以 Sheet《微波周报数据》「流程版本」子表为准；git tag 作记录（当前 v0.5.11）。版本号三段式「发布.大修.小更」：发布段=对外发布计数（当前 0，未发布）；大修段=影响使用的改动；小更段=不影响使用的改动（进位规则见「二A」）。腾讯文档备份链接见本地 manifest.json（仅备份/参考，权威以本仓库为准）
